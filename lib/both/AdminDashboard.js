...
AdminDashboard = { // eslint-disable-line
  ...
  isGroupAdmin, getAdminGroup,
  groupSchemaProp: {
    type: String,
    index: true,
    // :TODO: Also have auto value set this for non-super admins
    autoform: {
      type: 'select',
      disabled () {
        return getAdminGroup(Meteor.userId()) !== 'super'
      },
      options () {
        // groups are restricted in publication
        return Groups.find().map(group => {
          return {label: group.name, value: group.slug}
        })
      }
    },
    autoValue () {
      const group = getAdminGroup(Meteor.userId())
      if (group === 'super') {
        return this.value
      } else {
        return group
